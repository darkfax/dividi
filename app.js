...
// Frazioni con numeratore > 1 (es: 2/3 di 12)
function qFrazioneNumeratore(){var dens=[2,3,4,5,6];var d=dens[ri(0,dens.length-1)];var n=ri(1,d-1);var mult=ri(2,8);var tot=d*mult;var result=n*mult;var fh='<span class="frac"><span class="fnum">'+n+'</span><span class="fden">'+d+'</span></span>';var objs=['caramelle','figurine','mele','stelle','biscotti','monete'];var obj=objs[ri(0,objs.length-1)];var templates=['Quanto è '+fh+' di <span class="num">'+tot+'</span> '+obj+'?','Hai <span class="num">'+tot+'</span> '+obj+'. Ne prendi '+fh+'. Quante ne hai?','Calcola '+fh+' di <span class="num">'+tot+'</span>.'];var opts=shuffle([result].concat(wrongAnswers(result,3,1,tot)));return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:templates[ri(0,templates.length-1)],options:opts.map(String),correct:String(result),explain:n+'/'+d+' di '+tot+' = ('+tot+' ÷ '+d+') × '+n+' = '+mult+' × '+n+' = '+result}}

// Frazioni equivalenti (es: 1/2 = ?/4)
function qFrazioneEquivalente(){var bases=[{n:1,d:2},{n:1,d:3},{n:1,d:4},{n:2,d:3},{n:3,d:4},{n:1,d:5},{n:2,d:5},{n:3,d:5},{n:4,d:5},{n:1,d:6},{n:5,d:6},{n:1,d:8},{n:3,d:8}];var base=bases[ri(0,bases.length-1)];var maxMult=base.d>=6?3:5;var mult=ri(2,maxMult);var sn=base.n*mult;var sd=base.d*mult;var style=ri(0,2);if(style===0){var correct=sn;var f1='<span class="frac"><span class="fnum">'+base.n+'</span><span class="fden">'+base.d+'</span></span>';var f2='<span class="frac"><span class="fnum">?</span><span class="fden">'+sd+'</span></span>';var opts=shuffle([correct].concat(wrongAnswers(correct,3,1,sd)));return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:'Completa: '+f1+' = '+f2,options:opts.map(String),correct:String(correct),explain:base.n+'/'+base.d+' = '+sn+'/'+sd+' (moltiplica per '+mult+')'}}else if(style===1){var correct2=sd;var f3='<span class="frac"><span class="fnum">'+base.n+'</span><span class="fden">'+base.d+'</span></span>';var f4='<span class="frac"><span class="fnum">'+sn+'</span><span class="fden">?</span></span>';var opts2=shuffle([correct2].concat(wrongAnswers(correct2,3,2,40)));return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:'Completa: '+f3+' = '+f4,options:opts2.map(String),correct:String(correct2),explain:base.n+'/'+base.d+' = '+sn+'/'+sd+' (moltiplica per '+mult+')'}}else{var correct3=base.n+'/'+base.d;var wrongPool=shuffle(bases.filter(function(b){return b.n!==base.n||b.d!==base.d})).slice(0,3).map(function(b){return b.n+'/'+b.d});var opts3=shuffle([correct3].concat(wrongPool));var fsn='<span class="frac"><span class="fnum">'+sn+'</span><span class="fden">'+sd+'</span></span>';return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:'Semplifica: '+fsn,options:opts3,correct:correct3,explain:sn+'/'+sd+' = '+base.n+'/'+base.d+' (dividi per '+mult+')',isText:true}}}

// Confronto tra frazioni (quale è più grande?)
function qFrazioneConfronto(){var style=ri(0,1);if(style===0){var d1=ri(2,6);var d2=d1;while(d2===d1)d2=ri(2,8);var bigD=d1<d2?d1:d2;var smallD=d1<d2?d2:d1;var bigger='1/'+bigD;var fA='<span class="frac"><span class="fnum">1</span><span class="fden">'+d1+'</span></span>';var fB='<span class="frac"><span class="fnum">1</span><span class="fden">'+d2+'</span></span>';var opts=shuffle(['1/'+d1,'1/'+d2,'Sono uguali','Non si può sapere']);return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:'Quale frazione è più grande? '+fA+' oppure '+fB+'?',options:opts,correct:bigger,explain:'Con lo stesso numeratore (1), il denominatore più piccolo dà la frazione più grande. 1/'+bigD+' > 1/'+smallD,isText:true}}else{var d=ri(3,8);var n1=ri(1,d-2);var n2=n1;while(n2===n1)n2=ri(1,d-1);var bigN=n1>n2?n1:n2;var bigger2=bigN+'/'+d;var fC='<span class="frac"><span class="fnum">'+n1+'</span><span class="fden">'+d+'</span></span>';var fD='<span class="frac"><span class="fnum">'+n2+'</span><span class="fden">'+d+'</span></span>';var opts2=shuffle([n1+'/'+d,n2+'/'+d,'Sono uguali',(n1+n2)+'/'+d]);return{type:'calc',badge:'🍕 Frazioni',cls:'badge-frac',html:'Quale frazione è più grande? '+fC+' oppure '+fD+'?',options:opts2,correct:bigger2,explain:'Con lo stesso denominatore ('+d+'), il numeratore più grande vince. '+bigger2+' è quella più grande.',isText:true}}}

// Vero o Falso: frazioni
function qTrueFalseFrazione(){var style=ri(0,3);if(style===0){var d=ri(3,8),n1=ri(1,d-2),n2=ri(1,d-1);if(n1===n2)n2=n2+1<d?n2+1:n2-1;var claim=ri(0,1)===0;var correct=claim===(n1>n2);return{type:'tf',html:n1+'/'+d+' è più grande di '+n2+'/'+d,correct:correct,explain:n1+'/'+d+(n1>n2?' > ':' < ')+n2+'/'+d+'. Stesso denominatore: confronta i numeratori.'}}else if(style===1){var d2=ri(2,6),mult2=ri(2,8),tot2=d2*mult2,res2=mult2;var isT=ri(0,1)===1;var shown=isT?res2:res2+ri(1,3)*(ri(0,1)?1:-1);if(shown<=0||shown===res2)shown=res2+2;return{type:'tf',html:'1/'+d2+' di <span class="num">'+tot2+'</span> = <span class="num">'+shown+'</span>',correct:isT,explain:'1/'+d2+' di '+tot2+' = '+tot2+' ÷ '+d2+' = '+res2+(isT?'. Esatto!':'. Non '+shown+'!')}}else if(style===2){var pairs=[{a:'1/2',b:'2/4',eq:true},{a:'1/3',b:'2/6',eq:true},{a:'1/4',b:'2/8',eq:true},{a:'1/2',b:'1/4',eq:false},{a:'2/3',b:'3/4',eq:false},{a:'1/3',b:'2/5',eq:false}];var pair=pairs[ri(0,pairs.length-1)];return{type:'tf',html:pair.a+' e '+pair.b+' sono frazioni equivalenti',correct:pair.eq,explain:pair.eq?(pair.a+' = '+pair.b+' sono equivalenti!'):(pair.a+' e '+pair.b+' NON sono equivalenti!')}}else{var d3=ri(2,6);var d4=d3+ri(1,4);var correct2=(1/d3)>(1/d4);return{type:'tf',html:'1/'+d3+' è maggiore di 1/'+d4,correct:correct2,explain:'1/'+d3+(correct2?' > ':' < ')+'1/'+d4+'. Denominatore più piccolo = frazione più grande.'}}}
...
// BUILD SETS
function buildClassica(){var qs=[];for(var i=0;i<2;i++)qs.push(qNumericDiv());qs.push(qQuanteVolte());qs.push(qGruppi());qs.push(qMoltODiv());qs.push(qTrappola());qs.push(qInversa());qs.push(qCatena());qs.push(qQualeCorretta());qs.push(qQuantiGruppi());qs.push(qConfronto());qs.push(qQuantiRestano());qs.push(qDivisoreNascosto());var fracGens=[qFrazione,qFrazioneNumeratore,qFrazioneEquivalente,qFrazioneConfronto];qs.push(fracGens[ri(0,fracGens.length-1)]());qs.push(fracGens[ri(0,fracGens.length-1)]());qs.push(qDuePassi());var extra=[qTempo,qSoldi,qProprieta];qs.push(extra[ri(0,2)]());return shuffle(qs).slice(0,15)}
function buildBlitz(){var qs=[];var gens=[qNumericDiv,qQuanteVolte,qInversa,qTrappola,qCatena,qQualeCorretta,qQuantiGruppi,qDivisoreNascosto,qFrazione,qFrazioneNumeratore,qFrazioneEquivalente,qFrazioneConfronto,qProprieta];for(var i=0;i<50;i++)qs.push(gens[ri(0,gens.length-1)]());return qs}
function buildVeroFalso(){var qs=[];for(var i=0;i<40;i++)qs.push(ri(0,2)===0?qTrueFalseFrazione():qTrueFalse());return qs}
function buildFrazioni(){var gens=[qFrazione,qFrazioneNumeratore,qFrazioneEquivalente,qFrazioneConfronto,qTrueFalseFrazione];var qs=[];for(var i=0;i<20;i++)qs.push(gens[ri(0,gens.length-1)]());return shuffle(qs)}
function buildBoss(){var qs=[];var gens=[qNumericDiv,qGruppi,qMoltODiv,qTrappola,qInversa,qCatena,qQualeCorretta,qQuantiGruppi,qConfronto,qQuantiRestano,qDivisoreNascosto,qFrazione,qFrazioneNumeratore,qFrazioneEquivalente,qFrazioneConfronto,qDuePassi,qTempo,qSoldi,qProprieta];for(var i=0;i<15;i++)qs.push(gens[ri(0,gens.length-1)]());return shuffle(qs)}
...
function startMode(mode){
  ...
  else if(mode==='frazioni'){G.questions=buildFrazioni();G.total=G.questions.length;document.getElementById('gameModeLabel').innerHTML='🍕 Sfida Frazioni'}
...
document.getElementById('modeFrazioni').addEventListener('click',function(){startMode('frazioni')});
